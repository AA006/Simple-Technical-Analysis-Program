...
while True:
    ...
    def veri_cek_func(path, sheet, col):
        workbook = load_workbook(filename=path)
        sheet = workbook[sheet]
        for cell in sheet[col]:
            bist.append(cell.value)
    def goto():
        return giris_func()
    def giris_func():
        ...
        if giris.upper() == "G":
            soru = input("Hazır BIST100 listesini taratmak için B, kendi hisselerinizi seçmek için K'yı tuşlayın-->")
            if soru.upper() == "B":
                veri_cek_func("veri.xlsx", "Sheet1", "I")
                for i in bist:
                    urun = i
                    hisse = TA_Handler(
                        symbol=urun,
                    ...
                    file_func(rating_func2(rating), urun.upper(), result_func(rating_func2(rating)), data1['low'],
                              data1['high'], veri_func_day())
                    rating = 0
